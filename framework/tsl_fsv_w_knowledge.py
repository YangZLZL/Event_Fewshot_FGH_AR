...
def val_epoch(query_data_loader, model, classifier, opt):
    ...
    with torch.set_grad_enabled(opt.grad_enabled_in_embeddin), torch.cuda.amp.autocast(enabled=opt.is_amp):
        ...
        query_labels = torch.arange(opt.test_way).repeat(opt.query).cuda() # [0,1,...,opt.test_way-1] * opt.query
        # set_trace()
        acc, pred, correct = count_accc(logits, query_labels)
        #print('correct:',correct)
        #print('iid:',iid)
        #print('sample_names:',sample_names)
        sample_accuracies = [(name, correct_.item()) for name, correct_ in zip(sample_names, correct)]
    output_file_path = '/media/yzl/sdb/ubunut_final_bakcup/21.44/shiyuheng/actionKnowledgeXfewshot/few-shot-video-classification/sample_details.txt'
    write_sample_details_to_file(sample_accuracies, output_file_path)
    return acc
def write_sample_details_to_file(sample_details, file_path):
    # Open the file in append mode ('a') to add content to the end if it already exists
    with open(file_path, 'a') as file:
        for sample_id, accuracy in sample_details:
            # Write sample ID and accuracy to the file, converting accuracy to string
            file.write(f"{sample_id} {accuracy}\n")
def count_accc(logits, labels):
    """
    Calculate the accuracy and predictions for each query sample.
    logits: Predictions from the model [batch_size, n_classes]
    labels: True labels for each query sample [batch_size]
    """
    pred = torch.argmax(logits, dim=1)
    correct = pred.eq(labels).float()  # Tensor indicating correctness of each prediction
    acc = correct.mean().item()
    return acc, pred, correct
...
if __name__ == '__main__':
   # mp.set_start_method('spawn')  # or 'forkserver'
    ...
    os.makedirs(os.path.join(opt.result_path, 'backup'),exist_ok=True)
    backup_files(
