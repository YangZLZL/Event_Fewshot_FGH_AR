    ...
    def __getitem__(self, index):
        ...
        if self.args.sample_mode=='dense':
            ...
            if self.args.CLIP_visual_fea_preload:
                ...
                #clip_visual_fea = torch.index_select(torch.load(osp.join(self.CLIP_visual_fea_dir, video_id)+'.pt', map_location=device).to(device), dim=0, index=torch.from_numpy(interpolated_indices).to(device))
                clip_visual_fea_path = osp.join(self.CLIP_visual_fea_dir, video_id) + '.pt'
                clip_visual_fea = torch.index_select(torch.load(clip_visual_fea_path, map_location='cpu'), dim=0, index=torch.from_numpy(interpolated_indices))
        else:
            raise
        # set_trace()
