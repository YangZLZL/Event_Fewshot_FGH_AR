...
class PCBlock4_Deep_nopool_res(nn.Module):
    ...
    def forward(self, x):
        ...
        #print('x.shape(in PCBlock4_Deep_nopool_res)', x.shape)
        x = self.ffn2(x)
        #print('xffn2.shape(in PCBlock4_Deep_nopool_res)', x.shape)
        return x
...
class SKUpdateBlock6_Deep_nopoolres_AllDecoder2(nn.Module):
    ...
    def forward(self, net, motion_hidden_state, inp, forward_corr, backward_corr, forward_flow, backward_flow, coords0, attention, bs):
        ...
        #print('motion_features(after MotionEncoder)', motion_features.shape)
        ...
        #print('before flow_head', net.shape)
        delta_flow = self.flow_head(net)
        #print('delta_flow(after flowhead)', delta_flow.shape)
